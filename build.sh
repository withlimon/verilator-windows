...
# ══════════════════════════════════════════════════════════════
echo "[4.5/9] Configuring with win32 threading..."
./configure --build="$BUILD" --host="$HOST" --prefix="$PKG" \
    --disable-pthreads \
    CFLAGS="$MAKE_CFLAGS" \
    CXXFLAGS="$MAKE_CXXFLAGS -std=c++11" \
...
#  FIX-24: Force C++11 standard throughout build
# ══════════════════════════════════════════════════════════════
echo "[5/9] Building (j=$J)..."

# Export build flags for the build process
export CXXFLAGS="$MAKE_CXXFLAGS -std=c++11"
...
patch_mk() {
    ...
    # Force C++11 standard to avoid ABI issues
    if grep -q "^CXXFLAGS" "$MK"; then
        if ! grep -q "-std=c++11" "$MK"; then
            sed -i '/^CXXFLAGS/s/$/ -std=c++11/' "$MK"
        fi
    else
        echo 'CXXFLAGS += -std=c++11' >> "$MK"
    fi
...
:: Run make with correct compiler variables and C++11 standard
...
TROUBLESHOOTING:
  ...
  "undefined reference to std::cxx11::basic_string"
    → C++11 ABI mismatch - this build is fixed with -std=c++11
...
echo "  FIX-24  Force C++11 standard + win32 threading (fixes ABI mismatch)"
echo "══════════════════════════════════════════════════════"