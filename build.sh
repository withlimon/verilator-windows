...
patch_mk() {
    ...
    # so PlatformIO/conda Python is NOT picked up accidentally
...
#    Tries python3, python, py -3 in order so it works regardless
#    of how Python was installed (system, conda, PlatformIO, etc.)
...
#    Ensures the CORRECT Python is used by PYTHON3 variable in make
...
#    No manual env var setup required by user
...
#    Prevents "g++ not found" in generated Makefile
...
:: Run make with correct compiler variables
...
grep -q "x86_64-w64-mingw32" "$PKG/include/verilated.mk" \
    && { echo "  [FAIL] verilated.mk has cross-compiler names!"; FAIL=1; } \
    || echo "  [PASS] verilated.mk — no cross-compiler names"

# DLL dependency check (should be minimal for static build)
echo "  [INFO] DLL dependencies:"
"$HOST-objdump" -p "$PKG/bin/verilator.exe" \
    | grep "DLL Name" | sed 's/^/          /'

# Static check: should NOT depend on libgcc/libstdc++ DLLs
"$HOST-objdump" -p "$PKG/bin/verilator.exe" \
    | grep -qi "libgcc\|libstdc++" \
